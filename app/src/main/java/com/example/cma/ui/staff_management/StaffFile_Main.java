    ...
    private void parseJSONWithGSON(String jsondata){
        ...
        if(array.equals("null")){
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(StaffFile_Main.this, "档案记录为空", Toast.LENGTH_LONG).show();
                }
            });
        }
        Gson gson = new Gson();
        List<StaffFile> newList = gson.fromJson(array.toString(),new TypeToken<List<StaffFile>>(){}.getType());
        list.clear();
        list.addAll(newList);
    }
